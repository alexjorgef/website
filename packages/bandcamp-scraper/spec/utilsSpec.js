...
describe(`utils`, function () {
  describe(`generateSearchUrl`, function () {
    ...
    it(`generate url without params.page`, function () {
      expect(utils.generateSearchUrl({ query: `mac demarco` })).toEqual(
        `https://bandcamp.com/search?q=mac%20demarco&page=1`
      )
    })

    it(`generates url with all params`, function () {
      expect(utils.generateSearchUrl({ query: `mac demarco`, page: 1 })).toEqual(
        `https://bandcamp.com/search?q=mac%20demarco&page=1`
      )
    })
  })
